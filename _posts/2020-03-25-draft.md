...
- Full integration with the new data structures and merge with Dynamodb, this is, refactoring the graph data structure and merge the new update versions microservice within, the versions function now works independently with the graph.
- Once that works, finish the adjustments for possible errors and bugs, and prepare the corresponding documentation for the new service.
...
- Define all the corresponding Python Classes and Objects that will be usd, as PyPy, CRAN and others sources, modify the codes presented in cf-scripts repo as the update functions main structure, more specifically,  deleting the old service and prepare all the necessary archives for a new upgrade.
...
- Once all the work is done, this is, all information relating the packages upgrade system, by the conda-forge bot,  is arranged, we are gonna start refactoring the `cf-scripts-update-function` to work totally independent from the graph structure, using the previous work on the `update-version` function and possibly begin the test for allocation on Dynamodb.
...
- Try a new idea for the packages organizations as a goal for efficiency when retrieving the new version, more accuracy and velocity. This idea is related to ho the bot reads the information relating to each package version, there are two main ideas here:
...
- Now, inspect the code for any possible issue, and formatting to final merge.
